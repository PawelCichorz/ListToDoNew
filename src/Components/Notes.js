...
function Notes(props) {
  ...
  async function fetchNotes(day) {
    const res = await axios.get(`http://localhost:3031/notes${day}`);
  ...
  async function deleteNote(id, day) {
    await axios.delete(`http://localhost:3031/notes${day}/${id}`);
    setNotesByDay((prevState) => ({
      ...prevState,
  ...
  async function AddNote(title, body, day) {
    const res = await axios.post(`http://localhost:3031/notes${day}`, {
      title,
      body,
    });
  ...
  async function Editnote(note, day) {
    ...
    notes[index] = note;
    await axios.put(`http://localhost:3031/notes${day}/${note._id}`, note);
    setNotesByDay((prevState) => ({
      ...prevState,
      [day]: notes,
    }));
